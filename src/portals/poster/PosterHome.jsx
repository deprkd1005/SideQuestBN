    ...
    <div className="app-content no-scrollbar" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div style={{ padding: '40px 24px 20px' }}>
        <div className="flex-between" style={{ marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '1.8rem', fontWeight: 900, fontFamily: 'Outfit' }}>Provider <span className="text-emerald">Hub</span></h1>
            <p style={{ color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.9rem' }}>Welcome back, {user?.fullname?.split(' ')[0]}</p>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            <button className="card-glass flex-center" onClick={() => navigate('/poster/notifications')} style={{ width: '48px', height: '48px', borderRadius: '16px', padding: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
          <button 
            ...
            onClick={() => navigate('/poster/post-service')}
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        <div className="card-glass" style={{ padding: '20px', marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg, var(--bg-card) 0%, var(--bg-tertiary) 100%)' }}>
          <div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: 900, color: 'var(--emerald)' }}>{activeOrders.length}</div>
          </div>
          <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: 'var(--emerald-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {activeOrders.map(order => (
              ...
              >
                <div className="flex-between" style={{ marginBottom: '12px' }}>
                  <span className="badge badge-emerald" style={{ fontSize: '0.8rem', padding: '6px 12px' }}>BND {order.service.price}</span>
                  <div className={`badge ${order.status === 'pending' ? 'badge-emerald' : 'badge-emerald'}`}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                        {[1,2,3,4,5].map(i => <Star key={i} size={8} fill="var(--emerald)" color="var(--emerald)" />)}
                      </div>
                    </div>
                  </div>
