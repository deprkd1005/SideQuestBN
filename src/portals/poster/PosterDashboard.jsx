    ...
    <div className="app-content no-scrollbar" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div style={{ padding: '40px 24px 20px' }}>
        <div className="flex-between" style={{ marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '1.8rem', fontWeight: 900, fontFamily: 'Outfit' }}>SideQuest<span className="text-emerald">.BN</span></h1>
            <p style={{ color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.9rem' }}>Find local help in Brunei</p>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            <button className="card-glass flex-center" onClick={() => navigate('/poster/notifications')} style={{ width: '48px', height: '48px', borderRadius: '16px' }}>
          ...
          {categories.map(cat => (
            <button 
              ...
              style={{
                padding: '10px 20px',
                borderRadius: '12px',
                background: selectedCategory === cat ? 'var(--emerald)' : 'var(--bg-card)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
            {filteredServices.map(service => (
              <motion.div 
                ...
                whileTap={{ scale: 0.98 }}
                className="card" 
                onClick={() => navigate(`/poster/service/${service.id}`)}
                style={{ background: 'var(--bg-card)', border: '1px solid var(--border-glass)' }}
              >
                <div className="flex-between" style={{ marginBottom: '12px' }}>
                  <span className="badge badge-emerald" style={{ fontSize: '0.8rem', padding: '6px 12px' }}>BND {service.price}</span>
                ...
                <div className="flex-between" style={{ background: 'rgba(16, 185, 129, 0.05)', padding: '12px', borderRadius: '12px', border: '1px solid rgba(16, 185, 129, 0.1)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.65rem', fontWeight: 600, color: 'var(--emerald)' }}>✓ Verified Pro</div>
                    </div>
                  </div>
